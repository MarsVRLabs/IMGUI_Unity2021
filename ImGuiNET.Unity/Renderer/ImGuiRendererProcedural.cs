...
namespace ImGuiNET.Unity
{
    ...
    internal sealed class ImGuiRendererProcedural : IImGuiRenderer
    {
        ...
        private ComputeBuffer _vtxBuf; // gpu buffer for vertex data
        private GraphicsBuffer _idxBuf; // gpu buffer for indexes
        private ComputeBuffer _argBuf; // gpu buffer for draw arguments
        private readonly int[] _drawArgs = new int[] { 0, 1, 0, 0, 0 }; // used to build argument buffer

        private static readonly ProfilerMarker s_updateBuffersPerfMarker =
            new ProfilerMarker("DearImGui.RendererProcedural.UpdateBuffers");

        private static readonly ProfilerMarker s_createDrawComandsPerfMarker =
            new ProfilerMarker("DearImGui.RendererProcedural.CreateDrawCommands");
        ...
        public void Initialize(ImGuiIOPtr io)
        {
            ...
            io.BackendFlags |=
                ImGuiBackendFlags
                    .RendererHasVtxOffset; // supports ImDrawCmd::VtxOffset to output large meshes while still using 16-bits indices
        ...
        private static void CreateOrResizeVtxBuffer(ref ComputeBuffer buffer, int count)
        {
            buffer?.Release();
        ...
        private static void CreateOrResizeIdxBuffer(ref GraphicsBuffer buffer, int count)
        {
            buffer?.Release();
            int num = ((count - 1) / 256 + 1) * 256;
        ...
        private static void CreateOrResizeArgBuffer(ref ComputeBuffer buffer, int count)
        {
            buffer?.Release();
            int num = ((count - 1) / 256 + 1) * 256;
        ...
        {
            IntPtr prevTextureId = System.IntPtr.Zero;
            var clipOffset = new Vector4(drawData.DisplayPos.x, drawData.DisplayPos.y, drawData.DisplayPos.x,
            ...
            cmd.SetViewProjectionMatrices(
                Matrix4x4.Translate(new Vector3(.5f / fbSize.x, .5f / fbSize.y,
            ...
            for (int n = 0, nMax = drawData.CmdListsCount; n < nMax; ++n)
            {
                ...
                for (int i = 0, iMax = drawList.CmdBuffer.Size; i < iMax; ++i, argOf += 5 * 4)
                {
                    ...
                    Vector4 clip = Vector4.Scale(drawCmd.ClipRect - clipOffset, clipScale);
                    if (clip.x >= fbSize.x || clip.y >= fbSize.y || clip.z < 0f || clip.w < 0f) continue;

                    if (prevTextureId != drawCmd.GetTexID())
                        _properties.SetTexture(_texID,
                            _texManager.GetTexture((int)(prevTextureId = drawCmd.GetTexID())));

                    _properties.SetInt(_baseVertexID,
                        vtxOf + (int)drawCmd.VtxOffset); // base vertex location not automatically added to SV_VertexID
                    cmd.EnableScissorRect(new Rect(clip.x, fbSize.y - clip.w, clip.z - clip.x,
                        clip.w - clip.y)); // invert y
                    cmd.DrawProceduralIndirect(_idxBuf, Matrix4x4.identity, _material, -1, MeshTopology.Triangles,
                        _argBuf, argOf, _properties);
                }
